...
def downfold_inside_brillouin_zone(
    ...
    chosen_plane=[1,1,1]):
    r"""
        check if any point in a list is inside the brillouin zone, generated by the primitive vectors given in input
        if a chosen plane is pointed out, the projection of the brillouin zone on that plane is considered
        Parameters
        ----------
        ...
        chosen_plane: (3) :|array_like| 1 to check along a specif primitive vector direction, 0 no to check  
        Returns
        -------
        k_point_list: (N, 3) :|array_like| kx,ky,kz (k points are given in cartesian coordinates)
        indices: (N) |array_like| indices pointing if a shift has happend or not (1 along b1, 2 along b2, 3 along b1 and b2 in case of a 2D plane)
        """
    ...
    print(k_points_list,k_points_list.ndim)
    if k_points_list.ndim > 1:
...
def symmetry_analysis(
    ...
    threshold_symmetry):
    r"""
    Given a list of k points and a fixed point with a certain weight, different point group symmetry operations are applied to the list of k points
    The origin weight is properly distributed between the points:
        the orbits of the symmetry operations acquire a certain percentage of the fixed point weight, which takes into account how many orbits are found 
        of the orbits one k point (from the k list) is chosen as representative, and the weight of the orbit is assigned only to it
    Parameters
    ----------
    k_fixed_point: (3) :|array_like| fixed point coordinates (kx,ky,kz), fixed point considered in the point-group symmetry operations
    ...
    threshold_symmetry: |float| threshold to recognize a symmetry
    Returns
    -------
    k_list: (N,4) :|array_like| list of k points coordinates with respcetive weights from the symmetry analysis
    """
...
def k_points_triangulation_2D(
    ...
    border_indices=[]):
    r"""
    the sparse k points (kx,ky,kz,w) (distributed over a 2D plane) are linked togheter through a triangulation procedure (Delaunay procedure)
    the so-built triangles are listed, each triangle is characterized by three indices pointing to a position in the k poins list  
    the ordering of the vertices is so to assure a cloack-wise path along the triangles

    the periodicity of the 2D-BZ is properly taken into account:
        the points at the right border or at the bottom border are considered twice in the triangulation
    ...
    Parameters
    ---------
    k_points_list: (N,4) |array_like| (kx,ky,kz,w) cartesian coordinates and weight
    brillouin_primitive_vectors_2D: (2,3) |array_like| columns: kx,ky,kz, rows: b1,b2 
    precedent_count: |int| taking into account if the k points need to be added to a preceding list, so numbering properly the vertices, in order to be able to use them with the new total list
    border_indices: |list| empty list

    Returns
    ---------
    k_points_list: (n,4) |array_like| (kx,ky,kz,w)
    triangles: (s,3) |array_like| each vertex corresponds to a position in the k point list (taking into account the precedent count variable)
    border_indices: |list| indices of the points at the border (considered twice in the trangulation, in order to assure the periodicity)
    """
    number_elements=k_points_list.shape[0]
...
def local_refinment_with_symmetry_analysis(
    ...
    r"""
    Starting from a set of k points (old_k_points_list) with certain weights, to each k point iteratively (refinment_iterations) a new subset of k points (new_k_points_list) is associated,
    a symmetry analysis (symmetries) is performed on the subset, and consequently the initial weight of the k point is distributed over the subset of k points.
    
    This procedure goes on for the given number of refinment iterations.

    At each iteration for each k point four new k points are generated along the reciprocal primitive vectors of the selcted plane (normalized_brillouin_primitive_vectors_2d), at a distance from the original k point 
    equals to the refinment spacing (the refinment spacing is halved at each iteration).
    
    Obviously no refinment is applied to k points with null weight.

    Parameters
    ----------
    new_k_points_list: (,4) |array_like| list of values produced by the refinment procedure (expected to be empty at the beginning of the refinment procedure)
    old_k_points_list: (,4) |list| list of values give to the refinment procedure (kx,ky,kz,weight)
...
def k_points_generator_2D(
    ...
    Parameters
    ----------
    new_brillouin_primitive_vectors_3d: (3,3) |array_like| (columns: kx,ky,kz, rows: b1,b2,b3)
    old_brillouin_primitive_vectors_3d: (3,3) |array_like| (columns: kx,ky,kz, rows: b1,b2,b3)
    ...
    grid_spacing: |double| spacing of the k grid before the refinment procedure
    ...
    symmetries: |list of lists| a symmetry is a list of 3 elements (the versor is the axis of rotation, while the modulus is the angle)
    refinment_spacing: |double| initial spacing given to generate the refinment, half of the preceding refinment spacing is considered at each refinment iteration
    refinment_iterations: |int| number of refinment iterations considered
    threshold_symmetry: |double| therhold to recognize a symmetry
    symmetry_analysis: |boolean| local_refinment_with_symmetry_analysis or not
    precedent_count: |int|: if the k points produced have to be added to a preceding list (old_k_points_list.shape[0]!=0) -> thid variable is used when this function is called in the dynamical refinment
        in order to add new points to a preceding list
    Return
    -------------
    covering_BZ=True:
        symmetry_analysis=False:
...
def dynamical_refinment_little_paths_2D(
        ...
        default_gridding=100):
    r"""
    k points of the k points list are selected if they are vertices of the little paths to refine,
    a procedure takes into account if the different little paths to refine can be considered separately or not
    moreover, the procedure takes into account if the selected subset of k points in the parallelogram case, doesn't produce a not-parallelogram-like dominion

    a symmetry-local refinment is applied to the selected k points in case of triangles (number_vertices=3) 
    in case of parallelograms (number_vertices=4) a refinment considering only the border of the selected subset of k points is considered:
        a simil-BZ is built and the function to cover a BZ with parallelograms is used (k_points_generator_2D)
    ...
    if number_vertices==3:
        ...
        for i in range(number_eigenspaces):
            ...
            for j in range(number_eigenvectors_around):
                ...
                if shift==3:
                    eigenvectors_around_array[j,:3]=k_points_list[eigenspaces_k_points_around[i][j][0],:3]+brillouin_primitive_vectors_2d[1]+brillouin_primitive_vectors_2d[0]
                    any_shifting.append(3)
                elif shift==2:
                    eigenvectors_around_array[j,:3]=k_points_list[eigenspaces_k_points_around[i][j][0],:3]+brillouin_primitive_vectors_2d[1]
                    any_shifting.append(2)
                elif shift==1:
                    eigenvectors_around_array[j,:3]=k_points_list[eigenspaces_k_points_around[i][j][0],:3]+brillouin_primitive_vectors_2d[0]
                    any_shifting.append(1)
                else:
            ...
            ### by the clustering procedure two subgroups of points are obtained, one of them is then properly shifted to generate a single group of points
            ...
            print("after shifting in the origin",origin, k_points_list_projections_border)
...
def printing_covering_BZ_2D(
    ...
    r"""
    function to plot the k points and the associated little paths in the chosen 2D plane
    Parameters
    ----------
    brillouin_primitive_vectors_3d: (3,3) |array_like| (columns: kx,ky,kz, rows: b1,b2,b3)
    chosen_plane: (,3) |array_like|  ex. [0,1,1] the plane is the b2 x b3 plane in the reciprocal space where the bi are the brillouin primitive vectors
    little_paths: (s,number_vertices) |array_like| (v1,v2,v3...) int (v=vertex)
    k_points_list: (n,4) |array_like| (kx,ky,kz,w)
    little_paths: (n,3/4) |array_like| each row is a triangle/parallelogram, where the 3/4 vertices are integers, which correspond to the list positions of the given k points 
    number_vertices: |int| distinguishing between traingles and parallelograms
    """
...
##    position_little_paths_to_refine=[1]
