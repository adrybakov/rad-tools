...
def all_atoms_and_bonds(filename):
    ...
    for i in range(0, len(atoms) - 1):
        ...
        y1 = atoms[i][2]
        z1 = atoms[i][3]
        sphere(x1, y1, z1, r=radius[name1]/10,
               name=f'{name1}.{counter[name1]}')
        counter[atom[0]] += 1
        for j in range(i + 1, len(atoms)):
            ...
            if {name1, name2} == {"Ni", "S"} and distance(x1, y1, z1, x2, y2, z2, 3.299):
                cylinder_between(x1, y1, z1, x2, y2, z2, 0.05 * 2.58)
            if {name1, name2} == {"P", "S"} and distance(x1, y1, z1, x2, y2, z2, 2.851):
                cylinder_between(x1, y1, z1, x2, y2, z2, 0.05 * 2.58)


atoms = all_atoms_and_bonds(
    '/Users/rybakov.ad/Projects/Dalton/cover_art/final.txt')
